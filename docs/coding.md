...
### 1. 리팩토링은 AI에게 시키기 좋지만, 회귀(regression)는 사람이 잡아야 한다
"`submit_order` 안에 매칭 로직이 너무 길다, 책임 분리해줘"라고 AI에게 시켜 §4 표의 리팩토링을 짧은 시간에 완료할 수 있었다. 그러나 한 번은 `_match` 분리 과정에서 잔량 갱신 순서가 미묘하게 바뀌어 부분 체결 결과가 1만큼 어긋난 사례가 있었다. 단위 테스트(다음 단계)로 잡혔다.
→ **교훈: AI 리팩토링은 빠르지만 의미 보존을 보장하지 않는다. "결과 변경 없이"를 검증하려면 단위 테스트가 사전에 있어야 한다 — 즉, TDD가 바이브코딩과 가장 잘 맞는다.**

### 2. 정적 분석 친화적 코드는 AI가 잘 만들지만, 데드코드는 AI가 자주 남긴다
타입 힌트와 docstring은 AI가 빠짐없이 잘 추가한다. 그러나 초기 산출물에는 "혹시 필요할까봐" 만든 미사용 메서드(`get_best_bid()` 같은 것)가 끼어 있었다. 정적 분석 항목("데드코드 없는지")을 체크리스트로 돌리니 발견됐다.
→ **교훈: AI는 "충분히 만들기"에 강하고 "필요한 만큼만 만들기"에 약하다. 정적 분석 결과(또는 동등한 체크리스트)를 사후 검증 단계로 끼우지 않으면 코드베이스가 부풀려진다.**
